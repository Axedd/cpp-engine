...
IndexBuffer::IndexBuffer(uint32_t* data, int32_t count) : m_Count(count) {
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * sizeof(uint32_t), data, GL_STATIC_DRAW); // Here we copy the coordinates for the vertices over to the GPU memory
} 
